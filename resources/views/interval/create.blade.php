...
<div class="flex justify-center items-center mt-[50px] px-10">
    <div class="bg-white shadow w-full rounded-md p-4">
        <h2 class="font-semibold text-xl">Tambah Data Interval Pengguna</h2>
        <form action="{{route('interval.store')}}" enctype="multipart/form-data" method="post">
            @csrf
            <div class="w-full mt-5">
                ...
                <div class="flex flex-col gap-1 mt-2">
                    ...
                    <input id="interval" name="interval" type="number" value="{{old('interval')}}" required placeholder="60" class="w-full p-1 pl-2 rounded border border-gray-300" />
                </div>
            </div>
