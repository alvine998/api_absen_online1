...
<div class="flex justify-center items-center mt-[50px] px-10">
    <div class="bg-white shadow w-full rounded-md p-4">
        <h2 class="font-semibold text-xl">Tambah Data Supervisor</h2>
        <form action="{{route('memberspv.store')}}" enctype="multipart/form-data" method="post">
            @csrf
            <div class="w-full mt-5">
                <div class="flex flex-col gap-1 mt-2">
                    <label for="spv_id">Supervisor</label>
                    <select name="spv_id" class="w-full p-1 border border-gray-300" id="spv_id">
                        <option value="">Pilih Supervisor</option>
                        @foreach ($spvs as $spv)
                        <option class="uppercase" value="{{$spv->id}}">{{$spv->name}}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="mt-5 flex justify-between items-center gap-2">
                <a href="/memberspv" class="w-full p-1 rounded text-center bg-red-500 text-white hover:bg-red-600 duration-200">Batal</a>
